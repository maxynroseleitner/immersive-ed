...
namespace DigitalRuby.WeatherMaker
{
    public class WeatherMakerConfigurationScript : MonoBehaviour
    {
        ...
        private void UpdateTimeOfDay()
        {
            ...
            TimeOfDayCategoryText.text = WeatherMakerScript.Instance.DayNightScript.TimeOfDayCategory.ToString();
        }
        ...
        private void Start()
        {
            ...
            {
                if (light.type != LightType.Directional && light.type != LightType.Area && !WeatherMakerScript.Instance.LightManagerScript.AutoAddLights.Contains(light))
                {
                    WeatherMakerScript.Instance.LightManagerScript.AutoAddLights.Add(light);
                }
            }
        }
        ...
        {
            if (WeatherMakerScript.Instance.FogScript.FogProfile != null)
            {
                // if fog is not active, set the start fog density to 0, otherwise start at whatever density it is at
                float startFogDensity = WeatherMakerScript.Instance.FogScript.FogProfile.FogDensity;
                float endFogDensity = (startFogDensity == 0.0f ? 0.002f : 0.0f);
