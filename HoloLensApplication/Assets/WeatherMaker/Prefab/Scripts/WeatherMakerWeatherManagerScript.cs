...
namespace DigitalRuby.WeatherMaker
{
    ...
    public class WeatherMakerPropertyTransition : ISerializationCallbackReceiver
    {
        ...
        private MemberInfo GetMemberInfo()
        {
            ...
            info = Target.GetType().GetRuntimeField(FieldName);
            if (info == null)
            {
                info = Target.GetType().GetRuntimeProperty(FieldName);
            }

#else

            MemberInfo[] members = Target.GetType().GetMember(FieldName, BindingFlags.GetField | BindingFlags.SetField |
        ...
        public void UpdateStartValue()
        {
            ...
            if (prop == null)
            {
                ...
                if (field != null)
                {
                    StartValue = field.GetValue(Target);
                }
            }
            else
            {
                StartValue = prop.GetValue(Target, null);
            }
        }
    ...
    public class WeatherMakerWeatherManagerScript : MonoBehaviour
    {
        ...
        private void CustomUpdater(float percent)
        {
            ...
            if (!currentTransition.FieldNames.Contains("FogDensity"))
            {
                ...
                {
                    WeatherMakerScript.Instance.FogScript.FogProfile.FogDensity = Mathf.Lerp(startFogDensity, 0.0f, Mathf.Min(1.0f, percent * 2.0f));
                }
            }
